...
from ..dist import get_sequence_parallel_rank, get_sequence_parallel_world_size, get_sp_group
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(self, x, e, seq_lens, grid_sizes, freqs, context, context_lens, dtype=torch.float32):
        r"""
        Args:
            x(Tensor): Shape [B, L, C]
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def enable_teacache(
        self, coefficients, num_steps: int, rel_l1_thresh: float, num_skip_start_steps: int = 0, offload: bool = True
    ):
        ...
        self.freqs = torch.cat(
            [
                get_1d_rotary_pos_embed_riflex(
                    1024, self.d - 4 * (self.d // 6), use_real=False, k=k, L_test=L_test, L_test_scale=L_test_scale
                ),
    ...
    def forward(
        ...
        if self.teacache is not None:
            if not should_calc:
                previous_residual = (
                    self.teacache.previous_residual_cond if cond_flag else self.teacache.previous_residual_uncond
                )
